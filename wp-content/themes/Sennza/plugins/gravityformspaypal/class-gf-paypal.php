...
class GFPayPal extends GFPaymentAddOn {
	...
	protected function settings_trial_period( $field, $echo = true ) {
	...
	protected function get_product_query_string( $submission_data, $entry_id ) {
	...
	protected function get_donation_query_string( $submission_data, $entry_id ) {
		if ( empty( $submission_data ) ) {
			return false;
		}
	...
	protected function get_subscription_query_string( $feed, $submission_data, $entry_id ) {
	...
	protected function convert_interval( $interval, $to_type ) {
	...
	public function callback() {
		...
		if ( ! $entry ) {
			$this->log_error( "Entry could not be found. Entry ID: {$entry_id}. Aborting." );
	...
	protected function get_entry( $custom_field ) {
	...
	protected function can_process_ipn( $feed, $entry ) {
		...
		if ( strtolower( trim( $recipient_email ) ) != strtolower( trim( $feed['meta']['paypalEmail'] ) ) ) {
			$this->log_error( 'PayPal email does not match. Email entered on PayPal feed:' . strtolower( trim( $feed['meta']['paypalEmail'] ) ) . ' - Email from IPN message: ' . $recipient_email );
	...
	public function admin_edit_payment_status( $payment_status, $form_id, $lead ) {
	...
	protected function update_feed_id( $old_feed_id, $new_feed_id ){
		global $wpdb;
	...
	protected function add_legacy_meta( $new_meta, $old_feed ){
	...
	protected function update_payment_gateway() {
		global $wpdb;
	...
	protected function update_lead() {
		global $wpdb;
		$sql = $wpdb->prepare(
			"UPDATE {$wpdb->prefix}rg_lead SET payment_status='Paid', payment_method='PayPal'
	...
	protected function copy_transactions(){
	...
	protected function get_old_feeds() {
		global $wpdb;
